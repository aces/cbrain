    ...
    case task.status
      ...
      when 'New'
        ...
        if action == :go
          ...
          task.status_transition(self.status, "Failed Setup Prerequisites")
      ...
      when 'Data Ready'
        ...
        if action == :go
          ...
          task.status_transition(self.status, "Failed PostProcess Prerequisites")
      ...
      when /^Recover (Setup|Cluster|PostProcess)/
        ...
        if !canrecover
          ...
          task.status_transition(self.status, "Failed To Setup")       if     fromwhat    == 'Setup'
          task.status_transition(self.status, "Failed On Cluster")     if     fromwhat    == 'Cluster'
          task.status_transition(self.status, "Failed To PostProcess") if     fromwhat    == 'PostProcess'
          ...
          if fromwhat == 'Cluster' # special case, we need to resubmit the task.
            begin
              ...
              task.status_transition(self.status, "Failed On Cluster")
            end
          else # simpler cases, we just reset the status and let the task return to main flow.
            task.status_transition(self.status, "New")                    if fromwhat    == 'Setup'
            task.status_transition(self.status, "Data Ready")             if fromwhat    == 'PostProcess'
          end
        end
        task.save
      ...
      when /^Restart (Setup|Cluster|PostProcess)/
        ...
        if !canrestart
          ...
          task.status_transition(self.status, "Completed")
