	...
	def save_results #:nodoc:
		...
		if !params[:n_ra_DATA_FILES].blank? || !params[:p_ra_DATA_FILES].blank?
			...
			unless plsnpairsresult.save
				cb_error "Could not save back collection file '#{plsnpairsresult.name}'."
			end
			...
			if npairspls_result.save
				...
				self.addlog("Could not save back result file '#{file_basename}'.")
		...
		if  !params[:ne_cd_DATAMAT_BASENAME].blank?  
			...
			if npairspls_result1.save
				...
				self.addlog("Could not save back result file '#{datamat_basename}'.")	
			end
