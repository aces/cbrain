...
class ScirCloud < Scir
  ...
  class Session < Scir::Session
    ...
    def schedule_task_on_vm(task)
      ...
      raise "Another process is scheduling the task now" unless task.status_transition(current_status, "Scheduling") # ensures only one process can do the rest
      begin
        ...
        tvma.save!
        task.status_transition("Scheduling", current_status)
        return vm_id
      rescue => ex
        task.status_transition("Scheduling", current_status)
        raise ex
    ...
    def get_number_of_free_slots_in_vm(vm_task)
      ...
      n_tasks_in_vm = TaskVmAllocation.all.select { |x| 
        x.vm_id == vm_task.id.to_i &&
        active_task_ids.include?(x.task_id.to_i)
      }.count
