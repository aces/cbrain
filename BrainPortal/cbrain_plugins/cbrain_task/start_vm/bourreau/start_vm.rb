...
#      addlog "Elapsed time is #{elapsed}s, timeout is #{params[:vm_boot_timeout]}, timedout? #{elapsed > params[:vm_boot_timeout].to_f}"
  ...
  def mount_dir(remote_dir,local_dir)
    ...
    sshfs_command = "mkdir #{local_dir} -p ; sshfs -p 2222 -o follow_symlinks -o StrictHostKeyChecking=no #{user}@localhost:#{remote_dir} #{local_dir}" #TODO (VM tristan) put this 2222 somewhere in config
