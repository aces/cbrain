...
class BackgroundActivity::UnarchiveCollection < BackgroundActivity

  Revision_info=CbrainFileRevision[__FILE__] #:nodoc:

  def pretty_name
    num=self.items.size
    "Unarchive #{num} file#{num > 1 ? 's' : ''}"
  end
  ...
  def process(item)
    userfile = FileCollection.where(:archived => true).find(item)
    ...
    ok      = message.blank?
    message = ok ? "Unarchived: #{item}" : message
    [ ok, message ]
  end
