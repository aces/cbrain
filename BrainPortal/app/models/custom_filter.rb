...
class CustomFilter < ActiveRecord::Base
  ...
  def filter_scope(scope)
    scope = scope_name(scope)  unless self.file_name_type.blank? || self.file_name_term.blank?
    scope = scope_date(scope)  unless self.created_date_type.blank? || self.created_date_term.blank?
    scope = scope_size(scope)  unless self.size_type.blank? || self.size_term.blank?
    scope = scope_group(scope) unless self.group_id.blank?
    scope
  end
  ...
  def scope_date(scope)
    scope.scoped(:conditions  => ["DATE(userfiles.created_at) #{self.created_date_type} ?", self.created_date_term])
  end
  
  #Contruct the portion of the filter query which functions on 
  #the Userfile entry's size.
  def scope_size(scope)
    scope.scoped(:conditions  => ["userfiles.size #{self.size_type} ?", (self.size_term.to_i * 1000)])
  end
