...
class UserfilesController < ApplicationController
  ...
  def quality_control_panel #:nodoc:
    ...
    if @current_index >=  0 && commit_name && commit_name != :next && commit_name != :previous
      ...
      @current_userfile.set_tags_for_user(current_user, tag_ids) if tag_ids
      @current_userfile.update_attributes_with_logging(params[:userfile], current_user) if params[:userfile][:description]
    end
