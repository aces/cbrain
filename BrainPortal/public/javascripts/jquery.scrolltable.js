 ...
 * changes depending on whether or not the table has overflow
 * or not. The table can be setup to freeze a column once the
 * column comes to the left of the screen. The column will
 * unfreeze once the column to its right is fully visible.
 *
 * To setup the table without freezing a column simply call
 * the function ScrollTable on the table you want to apply
 ...
 *           column
 *
 * ie. $("#cand").ScrollTable({ "freezeColumn" : "pscid" });
 *
...
(function ($) {
    ...
    var setupScrolling = function (wrapper, rightLink, leftLink) {
        ...
        // Add wrapper necessary for scrolltable code
        $(table).wrap("<div class=\"scrollContentWrapper table-scroll\" style=\"overflow-x: auto\"></div>");
        ...
        // Remove wrapper necessary for scrolltable code
        $(table).unwrap();
    ...
    $.fn.ScrollTable = function (options) {
        this.filter("table").each(function () {
            ...
            if ($(this).parent(".scrollContentWrapper").length === 1) {
                unwrapTable(this);
            }
