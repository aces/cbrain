...
describe RemoteResource do
  ...
  describe "#send_command_clean_cache" do
    ...
    it "should raise an exception if older_than is not a number or time" do
      expect{ remote_resource.send_command_clean_cache(userfile_list, types, nil, younger_than) }.to raise_error(CbrainError)
    end
    it "should raise an exception if younger_than is not a number or time" do
      expect{ remote_resource.send_command_clean_cache(userfile_list, types,  older_than, nil) }.to raise_error(CbrainError)
    end
    it "should create a new clean_cache RemoteCommand" do
      ...
      remote_resource.send_command_clean_cache(userfile_list, types, older_than, younger_than)
    end
